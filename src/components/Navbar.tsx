...
const Navbar = () => {
  ...
  // Add event listener to close subcategory menu when clicking outside
  useEffect(() => {
  ...
  return (
    <>
      <nav className="bg-gradient-to-r from-blue-600 to-green-500 text-white shadow-lg sticky top-0 z-50" ref={navbarRef}>
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between h-16">
            {/* Logo and Brand */}
            <div className="flex items-center">
              ...
              >
                <span className="text-2xl font-bold">
                  <span className="text-white-200 text-2xl">Notes</span>
                  <span className="text-yellow-300 text-4xl">Market</span>
                </span>
              </Link>
            </div>
          ...
          >
            <div
              className={`fixed inset-y-0 left-0 w-64 bg-white transform transition-transform duration-300 ease-in-out overflow-y-auto ${
                isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'
              }`}
              onClick={e => e.stopPropagation()}
            >
                ...
                <div className="space-y-2">
                  ...
                  {categories.map((category) => (
                    <div key={category._id} className="border-b border-gray-100 last:border-b-0">
                      ...
                      {/* Mobile Submenu */}
                      {hoveredCategory === category._id && subCategories[category._id] && (
                        <div className="bg-gray-50 px-2 py-1 max-h-64 overflow-y-auto">
                          {subCategories[category._id].map((subItem) => (
                            <button
                              key={subItem._id}
                              onClick={() => handleSubItemClick(subItem._id)}
